...
Java_com_example_whensunset_ffmpeg_1learning_FFmpegPlayer_decode(JNIEnv *env, jobject instance,
                                                                 jstring input_jstr,
                                                                 jstring output_jstr) {
    ...
    int error_code = 0;
    if((error_code = avformat_open_input(&pFormatCtx,input_str,NULL,NULL))!=0){
        LOGE("Couldn't open input stream error_code = %d.\n", error_code);
        return -1;
    }
